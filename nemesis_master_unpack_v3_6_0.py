...
# -*- coding: utf-8 -*-
"""
nemesis_master_unpack_v3_6_0.py — Master driver for Zork Nemesis tools
...
- UX overhaul:
  ...
  * RAW mode prompt now explains "nemesis" vs "gi".
...
def _banner() -> str:
    ver_line = f"v{VERSION} — extract and convert Zork Nemesis / Grand Inquisitor assets"
    width    = 67
    ver_pad  = ver_line.center(width)
    return (
        f"{_GOLD}"
        f"   ╔{'═'*width}╗\n"
        f"   ║{'Z-VISION DECOMPRESSION TOOLKIT'.center(width)}║\n"
        f"   ║{'✶  For Zork: Nemesis and Zork: Grand Inquisitor  ✶'.center(width)}║\n"
        f"   ║{ver_pad}║\n"
        f"   ╚{'═'*width}╝"
        f"{_RESET}"
    )
...
def pick_types(d: Deps) -> List[int]:
    ...
    print(f"\n{_GOLD}  Select asset types to process{_RESET}"
          "  (comma-separated, or 0 = all available)\n")
...
def pick_formats(selected_types: List[int], d: Deps) -> Dict[int, str]:
    ...
    for t in selected_types:
        if t == 2:  # AVI
            ...
            print(
                f"\n{_GOLD}  RAW decode mode{_RESET}\n"
                "    nemesis  —  Zork: Nemesis\n"
                "    gi       —  Zork: Grand Inquisitor\n"
            )
            while True:
                sel = (input("  Choose for RAW [nemesis]: ").strip().lower() or "nemesis")
                if sel in ("nemesis", "gi"):
                    choices[t] = sel
                    break
                warn("Please enter 'nemesis' or 'gi'.")
        else:
            choices[t] = "default"
...
def _print_format_prompt(label: str, modes: set) -> None:
    print(f"\n{_GOLD}  {label} output format{_RESET}  (options: {', '.join(sorted(modes))})")
